...
const cacheEggInfo = {};
function getEggInfo(cwd, option = {}) {
    cacheEggInfo[cwd] = cacheEggInfo[cwd] || {};
    const cmd = `node ${path_1.default.resolve(__dirname, './scripts/eggInfo')}`;
    const opt = {
        cwd,
        env: Object.assign({}, process.env, { TS_NODE_TYPE_CHECK: 'false', TS_NODE_TRANSPILE_ONLY: 'true', TS_NODE_FILES: 'false', EGG_TYPESCRIPT: 'true', CACHE_REQUIRE_PATHS_FILE: path_1.default.resolve(config_1.tmpDir, './requirePaths.json') }, option.env),
    };
    const end = json => {
        caches.eggInfo = json;
